...
# --- REFERENCE DATABASES (CACHED FOR ULTRASPEED) ---
...
menu = st.sidebar.radio(
    ...
    [
        ...
        "⚙️ 5. Pasowania i Tolerancje ISO",
        "🔬 6. Właściwości i Klasy Stali",
        "⏱️ 7. Kalkulator Czasu Pracy"
    ]
)
...
if menu == "📊 Pulpit Główny":
    ...
    która w 100% odzwierciedla dane normatywne i zaawansowane formuły matematyczne zawarte w wersji arkuszowej (Excel v11).
    
    **Najnowsza odsłona v6.0 wprowadza:**
    *   **⚙️ Nowy moduł Pasowań i Tolerancji ISO 286-2:** Umożliwia precyzyjne wyliczenie odchyłek dla wałków i otworów oraz automatyczne określenie charakteru pasowania (luźne, mieszane, ciasne).
    *   **🪚 Kalkulator Cięcia i Strat (Scrap Estimator):** Pozwala na precyzyjne wyliczenie zapotrzebowania na sztangi profili (np. 6 m) oraz masę i koszt powstających odpadów (remnants i rzazu tarczy).
...
# PAGE 4: KALKULATOR CIĘCIA I STRAT
...
# PAGE 5: PASOWANIA I TOLERANCJE ISO (NEW)
elif menu == "⚙️ 5. Pasowania i Tolerancje ISO":
    st.markdown("<div class='main-header'>⚙️ 5. KALKULATOR PASOWAŃ I TOLERANCJI ISO 286-2</div>", unsafe_allow_html=True)
    st.markdown("<div class='sub-header'>Szybki dobór odchyłek granicznych i automatyczne wyznaczanie charakteru pasowań</div>", unsafe_allow_html=True)
    
    col_in, col_out = st.columns([1, 1])
    
    with col_in:
        st.subheader("Wprowadź wymiary i pasowania")
        nominal = st.number_input("Wymiar nominalny elementu (mm) (zakres 1 - 120 mm):", min_value=1.0, max_value=120.0, value=20.0, step=1.0)
        hole_class = st.selectbox("Klasa tolerancji otworu:", ["H7", "H8", "G7", "Brak"], index=0)
        shaft_class = st.selectbox("Klasa tolerancji wałka:", ["h6", "g6", "f7", "js6", "Brak"], index=0)

    with col_out:
        st.subheader("Wyniki dopasowania")
        ...
        if matched_range:
            st.info(f"**Wybrany zakres wymiarowy:** over {matched_range[0]} to {matched_range[1]} mm")
            ...
            # Fit determination
            fit_type = "Brak pasowania (Zdefiniuj obie klasy)"
            fit_color = "#333333"
            ...
            if hole_defined and shaft_defined:
                if hole_min >= shaft_max:
                    ...
                    fit_type = f"Luźne (Clearance Fit)  \nLuz minimalny: {luz_min:.1f} µm | Luz maksymalny: {luz_max:.1f} µm"
                    fit_color = "#27AE60"
                elif shaft_min >= hole_max:
                    wcisk_min = (shaft_min - hole_max) * 1000.0
                    wcisk_max = (shaft_max - hole_min) * 1000.0
                    fit_type = f"Ciasne (Interference Fit)  \nWcisk minimalny: {wcisk_min:.1f} µm | Wcisk maksymalny: {wcisk_max:.1f} µm"
                    fit_color = "#C0392B"
                else:
                    luz_gora = (hole_max - shaft_min) * 1000.0
                    wcisk_gora = (shaft_max - hole_min) * 1000.0
                    fit_type = f"Mieszane (Transition Fit)  \nMaksymalny luz: {luz_gora:.1f} µm | Maksymalny wcisk: {wcisk_gora:.1f} µm"
                    fit_color = "#F39C12"
            ...
            <div class='card'>
                <table style='width:100%; border: none;'>
                    <tr>
                        <td colspan='2' style='font-weight: bold; background-color: #ECEFF1; font-size: 1.1rem; color: #1F497D; padding: 5px;'>OTWÓR (Klasa: {hole_class})</td>
                    </tr>
                    <tr>
                        <td>Odchyłka górna ES / dolna EI:</td>
                        <td style='text-align: right;'>+{es_hole_um} / +{ei_hole_um} µm</td>
                    </tr>
                    <tr>
                        <td>Wymiary graniczne (maks / min):</td>
                        <td style='text-align: right; font-weight: bold;'>{hole_max:.4f} / {hole_min:.4f} mm</td>
                    </tr>
                    <tr>
                        <td>Pole tolerancji otworu (TD):</td>
                        <td style='text-align: right;'>{tol_hole} µm</td>
                    </tr>
                    <tr style='border-top: 1px solid #ccc;'>
                        <td colspan='2' style='font-weight: bold; background-color: #ECEFF1; font-size: 1.1rem; color: #1F497D; padding: 5px;'>WAŁEK (Klasa: {shaft_class})</td>
                    </tr>
                    <tr>
                        <td>Odchyłka górna es / dolna ei:</td>
                        <td style='text-align: right;'>{es_shaft_um:+} / {ei_shaft_um:+} µm</td>
                    </tr>
                    <tr>
                        <td>Wymiary graniczne (maks / min):</td>
                        <td style='text-align: right; font-weight: bold;'>{shaft_max:.4f} / {shaft_min:.4f} mm</td>
                    </tr>
                    <tr>
                        <td>Pole tolerancji wałka (Td):</td>
                        <td style='text-align: right;'>{tol_shaft} µm</td>
                    </tr>
                    <tr style='border-top: 2px solid #2F5496; background-color: #FFF2CC;'>
                        <td style='font-weight: bold; font-size: 1.1rem; color: #D35400;'>CHARAKTER PASOWANIA:</td>
                        <td style='text-align: right; font-weight: bold; font-size: 1.1rem; color: {fit_color};'>{fit_type}</td>
                    </tr>
                </table>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.error("Wymiar nominalny musi być w zakresie od 1 do 120 mm.")

# PAGE 6: WŁAŚCIWOŚCI I KLASY STALI
...
# PAGE 7: WYCENA CZASU PRACY
elif menu == "⏱️ 7. Kalkulator Czasu Pracy":
    st.markdown("<div class='main-header'>⏱️ 7. KALKULATOR CZASU PRACY & ROBOCIZNY</div>", unsafe_allow_html=True)
